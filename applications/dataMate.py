# This application provides assistance for clinical tests by generating forms for data input and analyzing inputted data.
# Based on basic settings of a test, the application generates an exact data form for the test.
# After finishing input data to the form, the application analyzes the data including statistical tests for normality.
...
class WindowClass(QMainWindow, form_class):
    ...
    def reset_loading(self):
        ...
        self.btn_anal.setEnabled(False)
        self.rbtn_halftest.setEnabled(0)
        self.rbtn_halftest.setAutoExclusive(0)
        self.rbtn_halftest.setChecked(False)
        self.rbtn_halftest.setAutoExclusive(1)
        self.rbtn_grouptest.setEnabled(0)
        self.rbtn_grouptest.setAutoExclusive(0)
        self.rbtn_grouptest.setChecked(False)
        self.rbtn_grouptest.setAutoExclusive(1)
        self.chbx_csv.setChecked(False)
    ...
    def btn_fload_function(self):
        ...
        if fname[0]:
            ...
            if not df_load.iat[0,0] in ['dm2.2', 'dm2.3']:
    ...
    def btn_anal_function(self):
        ...
        if self.det == 0:
            ...
            for para in self.parameters:
                ...
                if not self.parameters[order] in rc:
                    para.append([])
                    per_table = pd.DataFrame(index=piv_table0.index)
                    for pdct in anal_pdct:
                        for time in anal_time[1:]:
                            temp_series = (piv_table0.loc[:, ('%s_%s' % (pdct, time))] - piv_table0.loc[:, ('%s_%s' % (
                            pdct, anal_time[0]))]) / \
                                          piv_table0.loc[:, ('%s_%s' % (pdct, anal_time[0]))] * 100
                            temp_series = temp_series.rename('%s_%s(%%)' % (pdct, time))
                            per_table = pd.concat([per_table, temp_series], axis=1)
                    per_table = per_table.append(per_table.mean(), ignore_index=True)
                    per_table.index = list(piv_table0.index) + ['mean']

                    for r in dataframe_to_rows(per_table, index=True, header=True):
                        para.append(r)

                    for i in range(anal_pdct.size):
                        para.cell(row=12 + 2 * piv_table.index.size + i, column=1).value = anal_pdct[i]
                        for j in range(1, anal_time.size):
                            para.cell(row=12 + 2*piv_table.index.size + i, column=1 + j).value = \
                                per_table.loc['mean', '%s_%s(%%)' % (anal_pdct[i], anal_time[j])]
                            if i == 0:
                                para.cell(row=11 + 2*piv_table.index.size, column=1 + j).value = anal_time[j]

                    for cell in para[2*piv_table0.index.size + 11 + anal_pdct.size][0:]:
                        cell.font = Font(bold=True)
            ...
            for para in self.parameters:
                ...
                for group in groups:
                    ...
                    pvt1.index = list(np.array(gr0[(group,)].dropna(thresh = 1).index) + 1) + ['mean', 's-w', 'k-s']
                    ...
                    if self.parameters[order] in rc:
                        ...
                        per_table = pd.DataFrame(index=pvt.index)
                        for time in anal_time[1:]:
                            temp_series = (pvt.loc[:, time] - pvt.loc[:, anal_time[0]]) / \
                                          pvt.loc[:, anal_time[0]] * 100
                            temp_series = temp_series.rename(self.dict_time[time] + '(%)')
                            per_table = pd.concat([per_table, temp_series], axis=1)
                        per_mn = per_table.mean()
                        per_table = per_table.append([per_mn])
                        per_table.index = list(np.array(pvt.index) + 1) + ['mean']
                        pvt1 = pd.concat([pvt1, per_table], axis=1)
                    ...
                    for i in range(pvt1.columns.size):
                        para.cell(row=4 + order_group, column=4 + pvt1.columns.size + i).value = pvt1.loc['mean', :][i]
